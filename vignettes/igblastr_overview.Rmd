...
date: "Compiled `r BiocStyle::doc_date()`; Modified 2 April 2025"
package: igblastr
vignette: |
...
Note: `out` does not have columns indicating participant, time point, etc. so we
cannot stratify the below analysis by those sorts of variables. These analyses
therefore look at all sequences pooled together.
...
TODO: make a sequence logo plot of (kappa chain?) CDR3 AA sequences.

NOTE: currently there are no valid CDR3 sequences in the data.....

```{r}

# what are the unique CDR3 sequences in the data?
out %>% pull(cdr3) %>% unique()
out %>% pull(cdr3_aa) %>% unique()

# view percent of each column that has missing values
percent_missing <- round(100 * colSums(is.na(out)) / nrow(out), 0)
percent_missing[percent_missing > 0]
```

TODO: the following columns contain entirely missing values in `out` and need to be addressed:
